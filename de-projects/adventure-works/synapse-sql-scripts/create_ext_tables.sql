...
CREATE MASTER KEY ENCRYPTION BY PASSWORD = 'set your pass';

-- Create a database-scoped credential with a managed identity for authentication.
CREATE DATABASE SCOPED CREDENTIAL cred_shoaib
WITH 
...
WITH (
    LOCATION = 'https://shoaibadlsdev.dfs.core.windows.net/silver', -- URL to the external data storage location
    CREDENTIAL = cred_shoaib, -- Use the 'cred_shoaib' credential for authentication
);

-- Create another external data source named 'source_gold', pointing to the 'gold' directory in Azure Data Lake Storage.
CREATE EXTERNAL DATA SOURCE source_gold
WITH (
    LOCATION = 'https://shoaibadlsdev.dfs.core.windows.net/gold', -- URL to the external data storage location
    CREDENTIAL = cred_shoaib, -- Use the 'cred_shoaib' credential for authentication
);
